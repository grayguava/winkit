...
class CleanTool
{
    public static int Run()
    {
        ...
        // Stage 2: clean
        Etsu.WriteStep(2, 5, "Cleaning metadata...", "");
        logLines.Add("");
        logLines.Add("[2/5] Cleaning metadata");
        ...
        foreach (string file in files)
        {
            ...
            var psi = new ProcessStartInfo(Etsu.ExifPath, "\"-all=\" \"-overwrite_original\" \"-P\" \"-v\" \"" + tempFile + "\"")
            {
            ...
            string[] delLines = rawOutput.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            var deleted = new List<string>();
            foreach (string ln in delLines)
            {
                var m = System.Text.RegularExpressions.Regex.Match(ln.Trim(), @"^\s+Deleting\s+(.+)$");
                if (m.Success) deleted.Add(m.Groups[1].Value.Trim());
            }

            logLines.Add("  " + file);
            if (deleted.Count > 0)
                foreach (string tag in deleted) logLines.Add("    - " + tag);
            else
                logLines.Add("    (no metadata found)");
        }

        // Stage 3: verify
        Etsu.WriteStep(3, 5, "Verifying cleaned files...", "");
        logLines.Add("");
        logLines.Add("[3/5] Verifying cleaned files");
        ...
        logLines.Add("All wiped files verified OK");
        ...
        logLines.Add("");
        logLines.Add("All done. " + files.Length + " file(s) cleaned in place.");

        Etsu.WriteSep();
        Etsu.WriteStep(5, 5, "Done!", " - " + files.Length + " file(s) cleaned in place.");

        Etsu.WriteLog("clean", "SUCCESS", logLines);
    ...
    static bool VerifyFiles(string[] files, Dictionary<string, string> fileMap, string tempDir, List<string> logLines)
    {
        foreach (string file in files)
        {
            ...
            if (!File.Exists(tempFile))
            {
                ...
                logLines.Add("[ABORT] Temp file missing after wipe: " + file);
                return false;
            }
            ...
            if (new FileInfo(tempFile).Length == 0)
            {
                ...
                logLines.Add("[ABORT] Temp file is empty after wipe: " + file);
                return false;
            }
            ...
            catch
            {
                ...
                logLines.Add("[ABORT] Temp file unreadable after wipe: " + file);
                return false;
            }
        }
        return true;
